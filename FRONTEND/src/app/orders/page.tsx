...
const orders = [
  { id: 'SC-10234', customer: 'John Doe', date: '2026-04-01', total: 79.97, status: 'shipped', items: 2, method: 'M-Pesa' },
  { id: 'SC-10233', customer: 'Jane Smith', date: '2026-04-01', total: 49.99, status: 'processing', items: 1, method: 'Card' },
  { id: 'SC-10232', customer: 'Ali Hassan', date: '2026-03-31', total: 129.98, status: 'delivered', items: 3, method: 'COD' },
]
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-snap-border">
                    {['Order ID', 'Date', 'Items', 'Payment', 'Total', 'Status'].map((h) => (
                  ...
                  {orders.map((o) => (
                    <tr key={o.id} className="border-b border-snap-border hover:bg-snap-surface transition-colors">
                      ...
                      <td className="px-5 py-4">
                        <span className={`px-2 py-1 rounded-full text-xs font-semibold capitalize ${statusColors[o.status]}`}>
                          {o.status}
                        </span>
                      </td>
                    </tr>
